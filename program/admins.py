...
async def stop(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            clear_queue(chat_id)
            await m.reply_text(" ✅ تم ايقاف التشغيل بنجاح..")
        except Exception as e:
            traceback.print_exc()
            await m.reply_text(f"🚫 **error:**\n\n`{e}`")
    else:
        await m.reply_text("❌ لايوجد هناك اغنيه شغاله !.")
...
@Client.on_message(command(["تخطي", f"ت", "التالي"]) & other_filters)
@authorized_users_only
@check_blacklist()
async def skip(c: Client, m: Message):
    ...
    if queue == 0:
        ...
        await c.send_photo(
            ...
            caption=f"⏭ **تم بنجاح** التخطي الي المسار التالي.\n\n❤️‍🔥 **الاسم:** [{queue[0]}]({queue[1]})\n❤️‍🔥 **اެݪدࢪدشةه:** `{chat_id}`\n🦴 **طݪب اެݪحݪۅ:** {requester}",
        )
...
@Client.on_message(
    command(["ضبط", f"اظبط", "vol"]) & other_filters
)
@authorized_users_only
@check_blacklist()
async def change_volume(c: Client, m: Message):
    if len(m.command) < 2:
        return await m.reply_text("الاستخدام: `.اظبط` (`0-200`)")
...
async def cbstop(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await remove_active_chat(chat_id)
            clear_queue(chat_id)
            await query.edit_message_text("✅ تم ايقاف جميع الاغاني بنجاح.", reply_markup=close_mark)
        except Exception as e:
            traceback.print_exc()
            await query.edit_message_text(f"🚫 **error:**\n\n`{e}`", reply_markup=close_mark)
    else:
...
async def cbskip(_, query: CallbackQuery):
    ...
    queue = await skip_current_song(chat_id)
    if queue == 0:
        await query.answer("❌ لايوجد هناك اغنيه شغاله اصلا !.", show_alert=True)
    elif queue == 1:
        await query.answer("❌ لايوجد هناك اغنيه شغاله !.", show_alert=True)
    elif queue == 2:
        ...
        await _.send_photo(
            ...
            photo=image,
            reply_markup=InlineKeyboardMarkup(buttons),
            caption=f"⏭ **تم بنجاح** التخطي الى المسار التالي.\n\n❤️‍🔥 **Name:** [{queue[0]}]({queue[1]})\n❤️‍🔥 **Chat:** `{chat_id}`\n💡 **طلب من:** {requester}",
        )
        remove_if_exists(image)
